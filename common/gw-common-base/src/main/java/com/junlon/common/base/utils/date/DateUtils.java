package com.junlon.common.base.utils.date;
...
public class DateUtils extends org.apache.commons.lang3.time.DateUtils {
	...
	/**
	 * 判断一个时间是否在某个时间区间内，由于旧的方法 isBetween 无法正确判断，所以重新写一个
	 *
	 * @param now
	 *            目标时间
	 * @param start
	 ...
	 * @return 是否在区间内
	 */
	public static boolean isBetweenNew(Date now, Date start, Date end, int model) {
		return isBetweenNew(now, start, end, model, LEFT_OPEN_RIGHT_OPEN);
	}
	 ...
	 * @return
	 */
	public static boolean isBetweenNew(Date now, Date start, Date end, int compModel, int interModel){
		...
		long dateNumber = Long.parseLong(format.format(now));
	...
	public static long getDateDiff(String startDate, String endDate) {
		...
		try {
			...
			diff = (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) > 0 ? (date1.getTime() - date2.getTime())
					/ (24 * 60 * 60 * 1000) : (date2.getTime() - date1.getTime()) / (24 * 60 * 60 * 1000);
		} catch (ParseException e) {
		}
		return diff;
	}
	...
	public static long getDateDiff(Date date1, Date date2) {
		...
		long diff = (date1.getTime() - date2.getTime()) / (24 * 60 * 60 * 1000) > 0 ? (date1.getTime() - date2.getTime())
				/ (24 * 60 * 60 * 1000) : (date2.getTime() - date1.getTime()) / (24 * 60 * 60 * 1000);
		return diff;
	}
	...
	/**
	 * 给定日期Date，得出中国人习惯的星期数，即：1=周一，2=周二，3=周三，4-周四，5=周五，6=周六，7=周日
	 * @param date
	 * @return
	 */
	...
	public static long getTime(String dateStr){
		long time = 0;
		try{
			time = LONG_DATE_FORMAT.parse(dateStr).getTime();
		}catch(Exception e){
			logger.error(e);
		}
		return time;
	...
	/**
	 * 判断一个日期是不是比另一个日期晚(判断source是不是比target晚)
	 * @param source
	 * @param target
	 * @return
	 */
	public static boolean isLatter(Date source, Date target){
		return compareDate(source, target, Calendar.SECOND) > 0;
	}

	/**
	 * 判断一个日期是不是比另一个日期早(判断source是不是比target早)
	 * @param source
	 * @param target
	 * @return
	 */
	public static boolean isEarly(Date source, Date target){
		return compareDate(source, target, Calendar.SECOND) < 0;
	}
	 ...
	 * @return
	 */
	public static long getDaysMillSecond(int dayCount){
	 ...
	 * 取得sql的Time对象
	 * @param timeStr
	 * @return HH:mm:ss 给定时间的时分秒
	 */
	public static Time getSqlTime(String timeStr){
		if(StringUtil.isEmpty(timeStr)){
			return null;
		}
		Time time=null;
		try{
			Date date = HMS_FORMAT.parse(timeStr);
