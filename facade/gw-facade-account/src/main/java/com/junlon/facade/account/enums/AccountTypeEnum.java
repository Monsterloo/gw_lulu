...
public enum AccountTypeEnum {
	...
	public static List<Map<String, Object>> toMerchantList(){
		AccountTypeEnum[] arr = AccountTypeEnum.values();
		List<Map<String, Object>> list = new ArrayList<Map<String, Object>>();
		Integer[] merchantArr = {AccountTypeEnum.MERCHANT.getValue(), AccountTypeEnum.AGENT.getValue(), AccountTypeEnum.CUSTOMER.getValue(), AccountTypeEnum.PRIVATE.getValue()};
		for(AccountTypeEnum a : arr){
			if(Arrays.asList(merchantArr).contains(a.getValue())){
				Map<String, Object> map = new HashMap<String, Object>();
				map.put("value", a.getValue());
				map.put("desc", a.getDesc());
				list.add(map);
			}
		}
		return list;
